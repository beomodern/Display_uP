...
static uint8 I2C_M_slReadBuf[I2C_M_I2C_BTLDR_SIZEOF_READ_BUFFER];

/* Reads from this buffer */
static uint8 I2C_M_slWriteBuf[I2C_M_I2C_BTLDR_SIZEOF_WRITE_BUFFER];

/* Flag to release buffer to be read */
static uint32 I2C_M_applyBuffer;
...
static void I2C_M_I2CResposeInsert(void);


/*******************************************************************************
* Function Name: I2C_M_I2CCyBtldrCommStart
****************************************************************************//**
*
...
*
* \globalvars
*  I2C_M_applyBuffer - the flag to release the buffer with a response
*  to be read by the host.
*
*******************************************************************************/
void I2C_M_I2CCyBtldrCommStart(void)
{
    I2C_M_I2CSlaveInitWriteBuf(I2C_M_slWriteBuf, I2C_M_I2C_BTLDR_SIZEOF_WRITE_BUFFER);
    I2C_M_I2CSlaveInitReadBuf (I2C_M_slReadBuf, 0u);

    I2C_M_SetCustomInterruptHandler(&I2C_M_I2CResposeInsert);
    I2C_M_applyBuffer = 0u;
...
/*******************************************************************************
* Function Name: I2C_M_I2CCyBtldrCommStop
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: I2C_M_I2CCyBtldrCommReset
****************************************************************************//**
*
...
*
* \globalvars
*  I2C_M_slRdBufSize - used to store slave read buffer size.
*  I2C_M_slRdBufIndex - used to store the current index within the
*  slave read buffer.
...
/*******************************************************************************
* Function Name: I2C_M_I2CCyBtldrCommRead
****************************************************************************//**
*
...
* \globalvars
*  I2C_M_slStatus  - used to store current status of I2C slave.
*  I2C_M_slWriteBuf - used to store received command.
*  I2C_M_slWrBufIndex - used to store current index within slave
*  write buffer.
*
*******************************************************************************/
cystatus I2C_M_I2CCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        while (0u != timeoutMs)
        {
            ...
            if (0u != (I2C_M_I2C_SSTAT_WR_CMPLT & I2C_M_slStatus))
            {
                /* Copy number of written bytes */
                *count = (uint16) I2C_M_slWrBufIndex;
                ...
                (void) memcpy((void *) pData, (const void *) I2C_M_slWriteBuf,
                                              I2C_M_BYTES_TO_COPY(*count, size));
...
/*******************************************************************************
* Function Name: I2C_M_I2CCyBtldrCommWrite
****************************************************************************//**
*
...
*
* \globalvars
*  I2C_M_applyBuffer - the flag to release the buffer with
*  to be read by the host.
*
*******************************************************************************/
cystatus I2C_M_I2CCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        *count = size;
        (void) memcpy((void *) I2C_M_slReadBuf, (const void *) pData, (uint32) size);

        /* Read buffer is ready to be released to host */
        I2C_M_applyBuffer = (uint32) size;
...
/*******************************************************************************
* Function Name: I2C_M_I2CResposeInsert
****************************************************************************//**
*
...
static void I2C_M_I2CResposeInsert(void)
{
    if (I2C_M_CHECK_INTR_SLAVE_MASKED(I2C_M_INTR_SLAVE_I2C_ADDR_MATCH))
    {
        if (I2C_M_CHECK_I2C_STATUS(I2C_M_I2C_STATUS_S_READ))
        {
            ...
            if (0u != I2C_M_applyBuffer)
            {
                I2C_M_slRdBufSize  = I2C_M_applyBuffer;
                I2C_M_slRdBufIndex = 0u;
                I2C_M_applyBuffer  = 0u;
            }
        }
        else
        {
            ...
            {
                I2C_M_slRdBufIndex = I2C_M_slRdBufSize;
            }
        }
    }
}
